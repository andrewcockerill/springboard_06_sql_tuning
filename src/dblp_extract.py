...
def parse_article(dblp_path, save_path, save_to_csv=False, include_key=False):
    ...
    info = parse_entity(dblp_path, save_path, type_name, features,
                        save_to_csv=save_to_csv, include_key=include_key)
    ...
    with open(filename, 'wb') as file:
        ...
        if total is None:
            ...
            for data in response.iter_content(chunk_size=max(int(total/1000), 1024*1024)):
                ...
                sys.stdout.write(
                    '\r[{}{}] {}/{}'.format('█' * done, '.' * (50-done), size(downloaded), size(total)))
                sys.stdout.flush()
            sys.stdout.write('\n')
            # self.__log_msg('[*] Done!')
            is_downloaded = True
...
def main():
    ...
    parse_inproceedings(
        dblp_path, 'dataset/inproceedings.csv', save_to_csv=True)
